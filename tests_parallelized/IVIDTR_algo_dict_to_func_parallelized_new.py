...
def convert_dict_to_function(f_dict, default_value=0):
...
class VIDTR:
    ...
    def compute_optimal_policies(self):
        ...
        for t in range(self.MDP.time_horizon):
            ...
            modified_optimal_policy = convert_dict_to_function(optimal_policy_dicts[t],
                                                               default_value=
                                                               self.MDP.action_spaces[t][0])
            
            modified_optimal_value = convert_dict_to_function(optimal_value_dicts[t],
                                                              default_value =
                                                              0.0)
            
            optimal_policy_funcs.append(modified_optimal_policy)
            
            optimal_value_funcs.append(modified_optimal_value)
    ...
    def logger_subtract_dbus(sub_DBU):
        
        '''
        Updates the DBUs when subtracting one from the other
        ...
        if sub_DBU.no_of_boxes != 0 and necc_tuple not in new_condition_dicts:                                
            new_condition_dicts[necc_tuple] = 1                                 
            new_condition_DBUs.append(sub_DBU)                                  
                                                                                
    
    
    def subtraction_of_dbus(cond_DBU, optimal_cond_DBU):
        
        '''
        We subtract the optimal_cond_DBU from the cond_DBU
        ...
        print(f'We choose to subtract {cond_DBU} from {optimal_cond_DBU}')
    ...
    min_error = np.inf
    optimal_bellman_error = np.inf
    optimal_cond_dbu = None
    maxim_bellman_error_dict = {}
    fixed_bellman_error_dict = {}
    
    def logger_error(return_dict):                                             
        
        '''
        Check the errors for the logger

        Returns:
        -----------------------------------------------------------------------
        Updates the optimal bellman errors, total errors, bellman error dict and
        fixed bellman error dicts

        '''
        
        tupled_condition = return_dict['tupled_condition'][0]                  
        if return_dict['Is_tuple_present'][0] == False:                        
            maxim_bellman_error_dict[tupled_condition] = return_dict['Error'][0]
         
        for a in return_dict['Current_action_space'][0]:                       
            
            tupled_condition_a = return_dict['Tupled_cond_action']              
            if return_dict[f'Is_tupled_cond_action_present_{a}'] == False:     
                fixed_bellman_error_dict[tupled_condition_a] = return_dict['Total_Bellman_Error'][0]
                                                                                        
        if return_dict['Error'][0] < min_error:
            min_error = return_dict['Error'][0]                                 
            optimal_bellman_error = return_dict['Total_Bellman_Error'][0]       
            optimal_cond_dbu = return_dict['Cond_DBU'][0]                      
        
    
    def compute_errors(self, cond_index, t, condition_DBUs,                    
                       integration_method, integral_percent,
                       conditions, remaining_space):
        
        '''
        For a given condition compute the total_error and total_bellman_error
        
        Parameters:                                                             
        -----------------------------------------------------------------------
        cond_index : int                                                       
                     The index of the conditional DBUs                          
        
        t : float
            Timestep we are at
            
        condition_DBUs : list
                         List of conditions in their DBU form
        
        integration_method : func                                              
                             The method with which we wish to integrate functions with
        
        integral_percent : float                                               
                           The percent of points we wish to sample from
        
        conditions : list
                     The list of conditions we wish to compute the errors over
        
        remaining_space : DisjointBoxUnion
                          The space remaining over which we need to compute the integral
        
        Returns:
        -----------------------------------------------------------------------
        total_error : float
                      The total error computed for this timestep and index
                                                                                  
        total_bellman_error : float
                              The total bellman error for this timestep and index
                                                                                              
        '''
        
        cond_DBU = condition_DBUs[cond_index]                                   
        ...
        #print('We reach self.compute errors...hurray')
        
        error = 0.0                                                             
        tupled_condition = cond_DBU.dbu_to_tuple()
        is_tuple_present = tupled_condition in maxim_bellman_error_dict
        return_dict = {'Is_tuple_present': [is_tuple_present],
                       'Tupled_condition': [tupled_condition]}
        
        
        print('Hey we reach self.compute errors')
        raise Exception('Exception has been raised')
        
        if is_tuple_present is False:
            
            
            def maxim_bellman_function(s):
                return self.maximum_over_actions(self.bellman_equation(t), t)(s)
            ...
            #maxim_bellman_error_dict[tupled_condition] = maxim_bellman_error
        
        else:
            maxim_bellman_error = maxim_bellman_error_dict[tupled_condition]
        
        for a in self.MDP.action_spaces[t]:                                     
            
            if type(a) == type([0,1]) or type(a) == type(np.array([0,0])):
                a = list(a)
                tupled_cond_action = tuple(list(tupled_condition) + a)
                #ic(key)
            else:
                tupled_cond_action = tuple(list(tupled_condition) + a)              
            
            is_tupled_cond_action_present = tupled_cond_action in fixed_bellman_error_dict
            
            return_dict.update({f'Is_tupled_cond_action_present_{a}': [is_tupled_cond_action_present]})
            return_dict.update({'Tupled_cond_action': [tupled_cond_action]})
            
            
            if is_tupled_cond_action_present is False:
                def fixed_bellman_function(s):
                    return -VIDTR.fix_a(self.bellman_equation(t), a=a)(s)
                ...
                #fixed_bellman_error_dict[tupled_cond_action] = fixed_bellman_error
            
            else:
                fixed_bellman_error = fixed_bellman_error_dict[tupled_cond_action]
            ...
            return_dict.update({'Error': [error]})
            return_dict.update({'Bellman_Error': [bellman_error]})
            return_dict.update({'Current_action_space': [self.MDP.action_space[t]]})
            return_dict.update({'Cond_DBU': [cond_DBU]})
    ...
    def handler(self, error):
        '''
        Error callback function to the first apply_async present in the class  
        ...
        for t in np.arange(self.MDP.time_horizon-1, -1, -1):
            ...
            for l in range(self.max_lengths[t]-1):
                ...
                for i in range(max_conditions):
                    ...
                    print(f'We are at condition number {i}')                    
                    return_dict = pool.apply_async(self.compute_errors,
                                                   args = (i, t, condition_DBUs,
                                                           integration_method,  
                                                           integral_percent,    
                                                           conditions,          
                                                           remaining_space, ),  
                ...
                for i, cond_DBU in enumerate(condition_DBUs):                   
                    ...
                    sub_DBU = pool.apply_async(self.subtraction_of_dbus,        
                                               args = (cond_DBU, optimal_cond_DBU, ),
                                               callback = self.logger_subtract_dbus)
            ...
            min_error = np.inf
            for a in self.MDP.action_spaces[t]:
                def integrating_function(s):
                    return self.maximum_over_actions(self.bellman_equation(t), t)(s) -VIDTR.fix_a(self.bellman_equation(t), a=a)(s) - self.constant_eta_function(t)(s,a)
...
g = convert_dict_to_function(f_dict, S)
for k in f_dict.keys():
