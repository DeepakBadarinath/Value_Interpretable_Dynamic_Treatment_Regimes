...
#%%
import IVIDTR_algo_dict_to_func_parallel_new as VIDTR_module
import markov_decision_processes as mdp_module
import disjoint_box_union_parallel
import constraint_conditions_parallel
import constraint_conditions_parallel as cc

#%%

importlib.reload(constraint_conditions_parallel)
importlib.reload(disjoint_box_union_parallel)
...
from IVIDTR_algo_dict_to_func_parallel_new import VIDTR
...
#%%
    ...
    '''                                                                        
    optimal_conditions, optimal_actions = grid_class.algo.compute_interpretable_policies(integration_method=
                                                                                         DBU.integrate_static)
...
for dimensions in DIMENSIONS:
    for max_lengths in MAXLENGTHS:
        ...
        l1 = len(errors_per_integ_method)
        l2 = len(labels)
        if l1 < l2:
            
            plot_confidence_intervals(errors_per_integ_method[:l2], 'Total Errors Per Method',
                                      labels[:l2], f'epm_dim_{dimensions}_maxl_{max_lengths}')
        
            plot_confidence_intervals(bellman_errors_per_integ_method[:l2],
                                      'Total Bellman Errors Per Method', labels[:l2],
                                      f'bpm_dim_{dimensions}_maxl_{max_lengths}')
        else:
            
            plot_confidence_intervals(errors_per_integ_method[:l1], 'Total Errors Per Method',
                                      labels[:l1], f'epm_dim_{dimensions}_maxl_{max_lengths}')
        
            plot_confidence_intervals(bellman_errors_per_integ_method[:l1],
                                      'Total Bellman Errors Per Method', labels[:l1],
                                      f'bpm_dim_{dimensions}_maxl_{max_lengths}')
