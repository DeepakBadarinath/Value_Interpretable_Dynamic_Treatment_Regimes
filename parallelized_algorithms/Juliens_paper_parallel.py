...
class DPTPA:
   ...
   transition_kernels : list[function(s' \in state_spaces[t], s \in state_spaces[t],
    ...
    def evaluate_tree(tree, cost_functions, states):
        """
        Computes the score of a single tree.
        """
        score = 0.0
        tree_function = tree.generate_tree_function(tree)
        
        for r in cost_functions:
            dbu_iter_class = disjoint_box_union.DBUIterator(states)
            dbu_iterator = iter(dbu_iter_class)
            for s in dbu_iterator:
                score += r(s, tree_function(s))

        return score, tree, tree_function
    
    
    
    def compute_optimal_trees(self, tree_count = 100):
        ...
        trees_to_evaluate = self.decision_tree_sets[-1].trees[:tree_count]
        cost_functions = self.decision_tree_sets[-1].cost_functions
        states = self.decision_tree_sets[-1].states

        best_score = -np.inf
        best_tree = None
        best_tree_function = None

        with mp.Pool(processes=mp.cpu_count()) as pool:
            eval_func = partial(evaluate_tree, cost_functions=cost_functions, states=states)
    
        # Use imap_unordered for progress tracking
        results = []
        for res in tqdm(pool.imap_unordered(eval_func, trees_to_evaluate), total=len(trees_to_evaluate), desc="Evaluating Trees"):
            results.append(res)

        # Select the best tree from results
        for score, tree, tree_function in results:
            if score > best_score:
                best_score = score
                best_tree = tree
        ...
        for t in reversed(range(self.time_horizon - 1)):
            ...
            def evaluate_tree(tree, t, cost_functions, states_t, states_t_next,
                              best_tree_function, gamma, transition_kernel_t,
                              optimal_value_next):
                """
                Evaluates one tree and returns the score along with the tree and its function.
                """
                tree_function = DecisionTree.generate_tree_function(tree)
                score = 0.0

                for r in cost_functions:
                    dbu_iter_class_prev = disjoint_box_union.DBUIterator(states_t)
                    dbu_iterator_prev = iter(dbu_iter_class_prev)
        
                for s_old in dbu_iterator_prev:
                    dbu_iter_class_next = disjoint_box_union.DBUIterator(states_t_next)
                    dbu_iterator_next = iter(dbu_iter_class_next)

                for s_new in dbu_iterator_next:
                    score += r(s_old, best_tree_function(s_old)) + \
                             gamma * transition_kernel_t(s_new, s_old, best_tree_function(s_old)) * \
                                 optimal_value_next(s_new)

                return score, tree, tree_function


            # Prepare arguments for parallel evaluation
            trees_to_evaluate = self.decision_tree_sets[t].trees[:tree_count]
            cost_functions = self.decision_tree_sets[t].cost_functions
            states_t = self.decision_tree_sets[t].states
            states_t_next = self.decision_tree_sets[t + 1].states
            transition_kernel_t = self.transition_kernels[t]
            optimal_value_next = optimal_values[-1]  # Assuming last element is relevant here
            ...
            with mp.Pool(processes=mp.cpu_count()) as pool:
                    ...
                    states_t=states_t,
                    states_t_next=states_t_next,
                    best_tree_function=best_tree_function,
                    gamma=self.gamma,
                    transition_kernel_t=transition_kernel_t,
                    optimal_value_next=optimal_value_next,
                    )
    
                    results = []
                    for res in tqdm(pool.imap_unordered(eval_func, trees_to_evaluate),
                                    total=len(trees_to_evaluate), desc="Evaluating Trees"):
                        results.append(res)
            ...
            def v(s):                                                           
                ...
                for r in self.decision_tree_sets[t].cost_functions:             
                    ...
                    for s_new in dbu_iterator_next:
                        value += r(s, best_tree_function(s)) + (self.gamma *  self.transition_kernels[t](s_new, s,best_tree_function(s)) * optimal_values[-1](s)) 
