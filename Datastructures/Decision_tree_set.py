...
from Trees import TreeNode, DecisionTree
...
class DecisionTreeSet:
    ...
    def __init__(self, dimension, state_bounds, actions, cost_functions,
        ...
        trees : list
                Each tree is of the type BinaryTree (a collection of nodes with
                                                     functionality to go either
                                                     left or right)
        '''
        ...
        if len(trees) != 0:
            ...
            self.trees = self.generate_trees(max_depth,
                                             max_complexity,
                                             state_differences,
                                             self.state_bounds)
    ...
    def generate_trees(self, max_depth, max_complexity,
                       state_differences, state_bounds,
                       curr_depth = 0):
        ...
        Returns:
        -----------------------------------------------------------------------
        trees : List of BinaryTree datastructures                              
                All the possible binary trees that can be generated with the   
                above hyperparameters                                          
        
        '''
        
        # Code for one node DTrees that end at this point
        for a in self.actions:
            BinaryTree(Node(dimensions=[0], signs=[+1], values=[10],            
                            is_leaf=True, action=a))                            
        
        # Recursive code for adding more nodes
        ...
        for k_tuple in k_tuples:
            ...
            for possible_val in possible_vals:                                                  
                for possible_signs in possible_two_tuples:                                         
                    ...
                    BinaryTree(Node(dimensions = k_tuple,                      
    ...
    def generate_k_tuples(d, j):
        ...
        """
        Generates all possible k-tuples for k <= j from the list               
        [1, 2, ..., d]                                                          
